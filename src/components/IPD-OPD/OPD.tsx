...
const OPDPatients: React.FC = () => {
  const [currentPage, setCurrentPage] = useState(1);
  const [itemsPerPage, setItemsPerPage] = useState(10);

  const [data, setData] = useState<OPDPatient[]>([]);
  const [filteredData, setFilteredData] = useState<OPDPatient[]>([]);
  ...
  useEffect(() => {
    const fetchIPDdata = async () => {
      const response = await postApi("hospital/opd_patient/list", {});
      if (response.status === 200) {
        setData(response.data.data.OPDPatient);
        setFilteredData(response.data.data.opdPatient);
      }
    };
  ...
  const handleConfirmDelete = async () => {
    if (selectedId) {
      try {
        const response = await postApi("hospital/opd_patient/remove", {
          id: selectedId,
        });
        if (response.status === 200) {
    ...
    <div className="mt-2">
      <div className="row mb-3">
        <div className="col-md-6 mb-2 mb-md-0">
          <SearchBar
            placeholder="Search by Patient, Email, or OPD NO"
      ...
      <div className="table-container">
                ...
                displayedItems.map((item: any) => (
                  <tr key={item._id}>
                    ...
                    <td>
                      <Button
                        type="button"
                        className="p-0 border-0 bg-transparent action_icn"
                        onClick={() =>
                          navigate(`/OPD-patients/view/${item._id}`, {
                            state: { item },
                          })
                        }
                      >
                ...
                <tr>
                  <td colSpan={7} className="text-center py-5">
                    ...
                    <span
                      ...
                      <h4>No OPD patient data available</h4>
                    </div>
                  </td>
                </tr>
              )}
        ...
        <ConfirmDialog
          ...
          message="Are you sure you want to delete this OPD patient?"
