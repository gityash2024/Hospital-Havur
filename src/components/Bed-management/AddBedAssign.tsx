...
const BedAssign = () => {
  ...
  const [assignDate, setAssignDate] = useState<Date | null>(new Date());
  ...
  const handleFormSubmit = async () => {
    ...
    const submitData = {
      ...
      bed: selectedBed,
      date: assignDate ? assignDate.toISOString() : "",
      description,
    };

    try {
      const response = await postApi("hospital/bed_assign/add", submitData);
      if (response.status === 200) {
  ...
  return (
    <div className="container">
      <div className="d-flex justify-content-between align-items-center mb-3">
        <h4 className="title_sm mb-0">New Bed Assign</h4>
        <button
          type="button"
