...
function AdvancePayments() {
  ...
  useEffect(() => {
    const fetchAdvancePaymentsdata = async () => {
      const response = await postApi("hospital/advance-payments/list", {});
  ...
  const handleConfirmDelete = async () => {
    if (selectedId) {
      try {
        const response = await postApi("hospital/advance-payments/remove", {
          id: selectedId,
        });
        if (response.status === 200) {
  ...
  const handleSearch = (query: string) => {
    ...
    const filtered = data.filter(
      (item: any) =>
        item.receiptNo.toLowerCase().includes(query.toLowerCase()) ||
    ...
    <div className="mt-2">
      <div className="row mb-3">
        ...
        >
          <Button
            ...
            onClick={() => navigate("/advance-payments/create")}
          >
            Add Advance Payment
          </Button>
        </div>
      ...
      <div className="table-container">
          ...
          <Table hover className="admin_table">
            ...
            <tbody>
              {displayedItems.map((item: any) => (
                <tr key={item._id}>
                  <td>
                    <div className="badge bg-light-info">{item.receiptNo}</div>
                  </td>
                  <td>
                    {item.patientId ? (
                  ...
                  <td>
                    <Button
                      ...
                      className="p-0"
                      onClick={() =>
                        navigate(`/advance-payments/edit/${item._id}`, {
                          state: { item },
                        })
                      }
                    >
        ...
        <ConfirmDialog
          ...
          message="Are you sure you want to delete this Advance Payments?"
