...
const AddBills = () => {
  ...
  const [items, setItems] = useState([
    { name: "", qty: "", price: "", amount: 0 },
  ]);
  const [totalAmount, setTotalAmount] = useState(0);
  const [data, setData] = useState<any[]>([]);

  const addItem = () => {
    setItems([...items, { name: "", qty: "", price: "", amount: 0 }]);
  };
  ...
  const handleSubmit = async () => {
    ...
    let response = await postApi("hospital/bills/add", data);
      ...
      <div className="content d-flex flex-column flex-column-fluid pt-7">
        <div className="container-fluid">
          <div className="d-md-flex align-items-center justify-content-between mb-5">
            <h1 className="mb-0 title_sm">New Bills</h1>
